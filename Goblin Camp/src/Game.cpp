...
void Game::Draw(Coordinate upleft, TCODConsole* buffer, bool drawUI) {
	...
	for (std::list<boost::weak_ptr<FireNode> >::iterator firei = fireList.begin(); firei != fireList.end(); ++firei) {
		if (firei->lock()) firei->lock()->Draw(upleft, buffer);
	}
	for (std::list<boost::shared_ptr<Spell> >::iterator spelli = spellList.begin(); spelli != spellList.end(); ++spelli) {
		(*spelli)->Draw(upleft, buffer);
	}
