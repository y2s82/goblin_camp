...
void Game::CreateItems(int quantity, ItemType type, Coordinate corner1, Coordinate corner2) {
	...
	for (int items = 0; items < quantity; ++items) {
		Coordinate location(
			Random::Generate(0, areaWidth - 1) - std::min(corner1.X(), corner2.X()),
			Random::Generate(0, areaLength - 1) - std::min(corner1.Y(), corner2.Y())
		);
...
void Game::Update() {
	...
	if (Random::Generate(0, UPDATES_PER_SECOND * 15) == 0 || refreshStockpiles) {
	...
	Map::Inst()->Naturify(Random::Generate(0, Map::Inst()->Width() - 1), Random::Generate(0, Map::Inst()->Height() - 1));
}
	...
	if (Map::Inst()->Walkable(location.X(),location.Y()) && Map::Inst()->Type(location.X(),location.Y()) == TILEGRASS && Random::Generate(0, 4) < 2) {
		...
		for (unsigned int i = 0; i < NatureObject::Presets.size(); ++i) {
			if (NatureObject::Presets[i].minHeight <= height &&
				...
				natureObjectQueue.push(std::pair<int,int>(Random::Generate(0, NatureObject::Presets[i].rarity - 1) + Random::Generate(0, 2), i));
		}
		...
		if (Random::Generate(0, 99) < rarity) {
			...
			for (int clus = 0; clus < NatureObject::Presets[chosen].cluster; ++clus) {
				int ax = location.X() + ((Random::Generate(0, NatureObject::Presets[chosen].cluster - 1)) - std::max(1, NatureObject::Presets[chosen].cluster/2));
				int ay = location.Y() + ((Random::Generate(0, NatureObject::Presets[chosen].cluster - 1)) - std::max(1, NatureObject::Presets[chosen].cluster/2));
